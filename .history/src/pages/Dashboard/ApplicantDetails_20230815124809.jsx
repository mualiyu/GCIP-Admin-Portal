...
function ApplicantDetails() {
  ...
  const handleDecision = async () => {
    ...
    console.log('hey')
    const selectedConcerns = document.querySelectorAll('#checkboxes-tags-demo li[aria-selected="true"]');
    const concernsArray = Array.from(selectedConcerns).map(selectedConcern => {
    return selectedConcern.textContent.trim();
  });
  const remarkInput = document.getElementById("remarkInput");
  const remark = remarkInput.value;
  // const selectedOption = document.getElementById("decided").value;
  const handleOptionChange = (selectedValue) => {
    // Perform any actions you want when the option is changed
    console.log("Selected Option:", selectedValue);
    setSelectedOption(selectedValue);
    // You can update any relevant state or perform other operations here
  };
