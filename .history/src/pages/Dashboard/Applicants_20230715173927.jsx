            ...
            <>
              ...
              {allApplicants.map((applicant, index)=>(
                  <tr key={applicant.id} >
                    ...
                    <td>
                        <button disabled={applicant?.isApproved == 1} style={{border: 'none', padding: '9px 22px', cursor: 'pointer' }}  onClick={() => updateApplicantStatus(applicant.id, applicant?.isApproved)}>
                        {applicant?.isApproved == 1 ? 'Approved' : 'Approve'}
                        </button>
