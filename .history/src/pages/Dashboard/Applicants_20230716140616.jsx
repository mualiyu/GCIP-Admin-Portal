...
export default function Applicants() {
  ...
  const handleDownload = (documentName, index) => {
    console.log(index);
    const url = `/api/admin${documentName}`; 
            ...
            <>
              ...
              {allApplicants.map((applicant, index)=>(
                  <tr key={applicant.id} >
                    ...
                    <td style={{
        color: applicant?.isApproved == 1 ? '#aabf10' : applicant.isApproved == 2 ? 'green' : 'red',
      }}
      >
                    ...
                    <td>
                      ...
                      {applicant.isApproved != 2
                       &&
                        <button style={{border: 'none', marginRight: 4, padding: '9px 22px', cursor: 'pointer' }} disabled={buttonLoading[index]}  onClick={() => updateApplicantStatus(applicant.id, 2, index)}>
