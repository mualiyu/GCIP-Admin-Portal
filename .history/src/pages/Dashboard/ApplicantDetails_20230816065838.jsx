...
// const history = useHistory();
  ...
  const getData = async () => {
    ...
    if (success) {
      ...
      let status = data?.data?.application?.application_decisions[data?.data?.application?.application_decisions.length - 1].status;
  ...
  const removeLastPathSegment = () => {
    console.log('hey')
    const segments = `/Programme/Application/Submissions/${programId}/Applicant/${applicant_id}`.split('/');
  ...
  return (
    <div className="review-container">
      ...
      <div style={{
                  ...
                  <sup> 
                    {applicationStatus == 1 ? 'Submitted': applicationStatus == 2 ? 'Queried' : applicationStatus == 3 ? 'Successful' : applicationStatus == 4 ? 'Unsuccessful' : applicationStatus == 5 ? 'Under Review' : 'Not Reviewed Yet'}
                  </sup>
...
{current?.application_decisions !== 0 && !loading && (
          <div className="my-60" style={{borderTop: 'thin dashed grey', padding: '20px 0'}}>
            <h2 className="review_title" style={{color: 'red', fontWeight: 900}}>Application Status Reviews</h2>
...
<p className="review_title"> No review yet</p>
                 ...
                 {current?.application_decisions.length > 0 && 
                  ...
                  >
                    ...
                    <tbody>
                      {current?.application_decisions?.map(
                (decision, index) => {
                  return (
                          ...
                          <tr key={decision.id}>
                            <td>{index + 1}</td>
                            <td>
                            {decision.status == 1 ? 'Submitted': decision.status == 2 ? 'Queried' : decision.status == 3 ? 'Successful' : 'Unsuccessful'}
                            </td>
...
<p>Status: {applicationStatus == 1 ? 'Submitted': applicationStatus == 2 ? 'Queried' : applicationStatus == 3 ? 'Successful' : applicationStatus == 5 ? 'Under Review' : 'Unsuccessful'}</p>
{/* 1 Submitted 
2 queried
3 successful
4 unsuccessful */}
        ...
        >
          {decisionOptions.map((option) => (
            ...
            <Autocomplete
              ...
              isOptionEqualToValue={(option, value) => option.concern === value.concern}
              ...
              style={{ width: '82%' }}
              onChange={(event, newValue) => handleMultiSelectConcernsChange(newValue)}
              renderInput={(params) => (
...
<div style={{marginTop: 20}}>
  ...
  id="remarkInput" cols="100" rows="10" style={{padding: 18}} placeholder="Add Remark"></textarea>
</div>
