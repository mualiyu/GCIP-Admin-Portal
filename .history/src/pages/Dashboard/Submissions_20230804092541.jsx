...
export default function Submissions() {
  ...
  const downloadDocumentsInZip = async (id, title) => {
    console.log(id);
    // const url = `https://api.grants.amp.gefundp.rea.gov.ng/api/admin/download/applicationDocuments?application=${id}`; 
    ...
    try {
      // Make a fetch request to the server to get the document
      const response = await fetch(`https://api.grants.amp.gefundp.rea.gov.ng/api/admin/download/applicationDocuments?application=${id}`); // Replace with your server endpoint
      ...
      link.href = url;
      link.download = `${title}.zip`; // Replace with the desired filename for the downloaded document
      link.click();
        ...
        <TableBody>
          {allSubmissions.map((applicant, index) => (
            ...
            >
              ...
              <TableCell> 
                <button style={{border: 'none', border: 'thin solid green', backgroundColor: 'white', color: 'green', marginRight: 4, padding: '9px 22px', cursor: 'pointer' }} disabled={buttonLoading[index]}  onClick={() => downloadDocumentsInZip(applicant.id, applicant?.applicant?.name)}>
                          {buttonLoading[index] ? 'Loading...' : 'Download'}
                        </button>
                        <button style={{border: 'none', backgroundColor: '#006439', border: 'none', color: 'white', marginRight: 4, padding: '9px 22px', cursor: 'pointer' }} disabled={buttonLoading[index]}  onClick={() => seeDetails()}>
                          {buttonLoading[index] ? 'Loading...' : 'See More'}
                        </button>
                </TableCell>
